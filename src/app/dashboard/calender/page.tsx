...
function Calendar() {
  ...
  <div className="flex justify-between items-center mb-6">
    ...
    <div className="flex items-center space-x-2">
      ...
      <button 
        onClick={() => setCurrentDate(new Date())}
        className="px-4 py-2 bg-primary text-white rounded-xl hover:bg-primary/80 transition-all text-sm font-medium shadow-sm"
      >
        Today
      </button>
  ...
  <div className="grid grid-cols-7 gap-3 mb-4">
    {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
      <div 
        key={day} 
        className="text-center text-sm font-semibold text-primary/70 py-2"
      >
        {day}
      </div>
    ))}
    ...
    {emptyDays.map((_, index) => (
      <div 
        key={`empty-${index}`} 
        className="h-24 rounded-xl bg-gray-50 border border-gray-100"
      />
    ))}
    ...
    {days.map(day => {
      ...
      return (
        <div 
          key={day} 
          className={`
            h-24 rounded-xl p-2 cursor-pointer transition-all border 
            ${
              isToday 
        ...
        >
          {/* Date & Badge */}
          <div className={`flex justify-between items-center mb-1 ${
            isToday ? 'text-primary font-semibold' : 'text-gray-800'
          }`}>
            <span>{day}</span>
          ...
          <div className="space-y-1">
            {dateTasks.slice(0, 2).map(task => (
              <div 
                key={task._id} 
                className={`
                  text-xs p-1 rounded-lg truncate border-l-4 
                  ${
                    task.priority === 'high' 
      ...
      {getTasksForDate(selectedDate).length === 0 ? (
        ...
        <div className="space-y-4">
          {getTasksForDate(selectedDate).map(task => (
            <div 
              key={task._id} 
              className="p-4 rounded-xl border border-gray-200 hover:shadow-lg transition-all bg-white"
            >
              <div className="flex justify-between items-start">
              ...
              <div className="flex items-center mt-3 text-sm text-gray-500 gap-3">
                <span>Patient: {task.patientName}</span>
