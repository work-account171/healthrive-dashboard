...
const filteredTasks = tasks.filter((task) => {
  ...
  if (query) {
    ...
    if (!titleMatch && !patientNameMatch) {
      return false;
    }
  }
  ...
  // Only return tasks that are overdue
  return isOverdue;
});
    ...
    <>
      ...
      <div className="flex justify-between items-center">
        <div className="flex flex-col gap-4 justify-start items-start mb-4">
          <h1 className="text-[32px] font-bold">Past Due Task Manager</h1>
          <p className="text-[16px]">
            These tasks are overdue, but you can still complete them. Take them
            one by one — you&apos;ve got this.{" "}
          </p>
        </div>
        {/* <button
      ...
      {/* Filter search bar */}
      <div className="rounded-xl mb-4 py-6 px-5 flex flex-col gap-5 border border-gray-100">
        ...
        <div className="flex justify-start items-start gap-6 w-full">
          ...
          <div className="flex justify-center items-center py-3 px-4 border border-red-600 text-[16px] text-red-600 bg-white rounded-xl w-40">
            Past Due
          </div>
          {/* <select
            value={dueDateFilter}
